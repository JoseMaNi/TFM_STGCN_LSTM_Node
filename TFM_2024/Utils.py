...
class BackUp:
    ...
    def __init__ (self, folder = '/content', filename = 'backup.pkl'):
        self.last_backup = None
        self.folder = folder
        self.filename = filename
        self.Bup_route = self.folder + '/' + self.filename
        self.vals = {}

    def Save (self, folder = None, filename = None, timestamp = True, overwrite = True):

        # print(Absroute,filename)
        if filename is None:
            # print(Absroute,filename)
            if timestamp:
                timestamp = datetime.datetime.now ().strftime ('%Y-%m-%d_%H-%M-%S')
                self.filename = f'backup_{timestamp}.pkl'
        else:
            self.filename = filename

        if folder is None:
            if self.last_backup is not None:
                print (f'Guardado por defecto {os.path.dirname (self.Bup_route)}')
                self.folder = os.path.dirname (self.Bup_route)
            else:
                self.folder = os.getcwd ()
        else:
            self.folder = folder

        Absname = self.folder + '/' + self.filename
        # Guardar todas las variables en un archivo
        self.last_backup = self.Bup_route
        self.Bup_route = Absname
        print ('Guardando en ', Absname)
        dill.dump_module (Absname)
        print ('Guardado en >>', self.folder, '>>', self.filename)
        if overwrite:
            try:
                os.remove (self.last_backup)
                print (f'El archivo {self.last_backup} ha sido borrado.')
            except FileNotFoundError:
                print (f'El archivo {self.last_backup} no existe.')

    def Restore (self, Absroute = None):
        if Absroute is None:
            Absroute = self.Bup_route

        dill.load_module (Absroute)
        print ('Restaurado desde', Absroute)

    def remove (self, Absroute = None):
        if Absroute is None:
            Absroute = self.Bup_route
            print (f'No se especifica fichero se toma {Absroute}')
        # Borrar el archivo
        try:
            os.remove (self.Bup_route)
            print (f'El archivo {self.Bup_route} ha sido borrado.')
        except FileNotFoundError:
            print (f'El archivo {self.Bup_route} no existe.')

    def save_val (self, val, valname = 'x', folder = None, timestamp = True):
        if timestamp:
            timestamp = datetime.datetime.now ().strftime ('%Y-%m-%d_%H-%M-%S')
            filename = f'backup_{valname}_{timestamp}.pkl'
        else:
            filename = f'backup_{valname}.pkl'
        if folder is None:
            folder = self.folder
        else:
            folder = os.path.dirname (self.Bup_route)
        Absname = folder + '/' + filename
        self.vals [valname] = Absname
        with open (Absname, 'wb') as f:
            dill.dump (val, f)
        print ('Guardado en ', Absname)

    def load_val (self, valname):
        route = self.vals [valname]
        with open (route, 'rb') as f:
            val = dill.load (f)
        print ('Restaurado desde', route)
        return val
